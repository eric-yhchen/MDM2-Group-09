...
def solver(kd,kp):
    ...
    sol = solve_ivp(metronome_matrix_system, t_span, y0, args=[kd, kp], t_eval=t_eval, method="RK45")
...
plt.suptitle("Varying derivative gain")
#plt.legend()
#plt.grid()

kds = [0.001, 0.01, 0.1]
for kd, ax in zip(kds, (ax1, ax2, ax3)):
    time, theta1, theta2 = solver(kd, 0.8)
    ax.set_title(kd)
