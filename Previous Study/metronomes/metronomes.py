...
from numpy import arange, array, concatenate, size, random
from matplotlib.pyplot import plot, figure, subplot, xlabel, ylabel, xlim, title, axis, draw, savefig, suptitle
...
def metronomes(state,t,params):
	...
	a1d = state[2]  # vel of pendulum 1
	a2d = state[3]  # vel of pendulum 2
	...
	x = state[4]   # position of cart
	xd = state[5]  # vel of cart
	...
	xdd_top = tmp1 + tmp2 - (Md/M)*xd - (Mk/M)*x
	xdd_bot = 1.0 - (m1/M)*cos(a1) - (m2/M)*cos(a2)
	xdd = xdd_top / xdd_bot
	...
	return [a1d, a2d, a1dd, a2dd, xd, xdd]
...
ar = random.uniform(-30, 30, 2)*pi/180
ard = random.uniform(-360, 360, 2)*pi/180
...
# plot results
figure(figsize=(8,10))
suptitle(f"Number of metronomes: {(len(state0) - 2)//2}")
subplot(3,1,1)
#plot(t,state[:,[0,1,2,3,4]]*180/pi)
plot(t,state[:,[0,1]]*180/pi)
ylabel('METRONOME ANGLES (rad)')
xlabel('TIME (sec) (6000 max)')
subplot(3,2,3)
#plot(t[0:200],state[0:200,[0,1,2,3,4]]*180/pi)
plot(t[0:200],state[0:200,[0,1]]*180/pi)
ylabel("First 200 timesteps")
subplot(3,2,4)
#plot(t[-200:-1],state[-200:-1,[0,1,2,3,4]]*180/pi)
plot(t[-200:-1],state[-200:-1,[0,1]]*180/pi)
ylabel("Last 200 timesteps")
subplot(3,1,3)
#plot(t,state[:,10])
plot(t,state[:,5])
...
#	figure()
	...
	c, = plot([b1x, b2x],[0,0],'k-',linewidth=lw)
	#xlim(min(state[:,10])-0.3, max(state[:,10])+.5)
	xlim(min(state[:,5])-0.3, max(state[:,5])+.5)
